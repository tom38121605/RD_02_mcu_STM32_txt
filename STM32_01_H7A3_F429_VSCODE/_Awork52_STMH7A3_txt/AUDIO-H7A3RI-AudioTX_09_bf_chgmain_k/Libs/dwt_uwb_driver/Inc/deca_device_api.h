...
// Automotive build: uncomment line below to enable hardenned PLL calibration for automotive application.
//#define AUTO_PLL_CAL
 ...
 * */
#ifdef AUTO_PLL_CAL
#define XTAL_TRIM_BIT_MASK 0x7F
#else
 ...
 */
    ...
    */
     ...
     * no return value
     */
    void dwt_read_tdoa_pdoa(dwt_pdoa_tdoa_res_t *result, int index);
    ...
    */
     ...
     */
    int dwt_setchannel(dwt_pll_ch_type_e ch);
     ...
     * returns DWT_SUCCESS if successful, otherwise returns < 0 if failed.
     */
    int dwt_setphr(dwt_phr_mode_e phrMode, dwt_phr_rate_e phrRate);
     ...
     * returns DWT_SUCCESS if successful, otherwise returns < 0 if failed.
     */
    int dwt_setdatarate(dwt_uwb_bit_rate_e bitRate);
     ...
     * returns DWT_SUCCESS if successful, otherwise returns < 0 if failed.
     */
    int dwt_setrxpac(dwt_pac_size_e rxPAC);
     ...
     * returns DWT_SUCCESS if successful, otherwise returns < 0 if failed.
     */
    int dwt_setsfdtimeout(uint16_t sfdTO);
     ...
     */
    uint8_t dwt_pll_ch9_auto_cal(uint32_t coarse_code, uint16_t sleep, uint8_t steps);
     ...
     * @param uint8_t sleep
     * @param uint8_t steps
     * @param uint8_t temperature
     *
     * output parameters
     *
     * return number of steps taken to lock the PLL or DWT_ERROR
     */
    uint8_t dwt_pll_ch5_auto_cal(uint32_t coarse_code, uint16_t sleep, uint8_t steps, uint8_t temperature);
     ...
     *
     * input parameters:
     * @param int8_t temperature    -- pass in TEMP_INIT (-127) to use on chip temperature sensor
     * @param uint8_t crystal_trim  -- pass in 0 if you want to use the calibration value from OTP
     * @param int8_t crystal_trim_temperature  temperature of the crystal for crystal_trim, if TEMP_INIT (-127) will assume 25C.
     * @param int32_t crystal_alpha * 2^22   scaled alpha value
     * @param int32_t crystal_beta  * 2^22   scaled beta value
     *
     * output parameters
     *
     * return newly programmed crystal trim value
     */
    uint8_t dwt_xtal_temperature_compensation(int8_t temperature, uint8_t crystal_trim, int8_t crystal_trim_temperature, int32_t crystal_alpha, int32_t crystal_beta);
